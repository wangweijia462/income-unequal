...
\begin{tablenotes}[flushleft]
\small
\item[] \raggedright \textit{Notes:} The aggregate four-region ROW welfare loss is 0.6165pp; the exporter-importer masking gap is 0.5776pp.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The fiscal exercise examines how different budgetary responses interact with war-driven sovereign spread widening. We compare three rules against the 130 percent debt-to-GDP sustainability threshold: passive automatic stabilizers, discretionary austerity triggered when the debt ratio exceeds the threshold, and targeted fiscal stimulus when the output gap turns negative. The exercise identifies whether fiscal activism can offset the demand contraction, and whether any rule can prevent debt sustainability from binding.
